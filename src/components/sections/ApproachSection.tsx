...
export function ApproachSection() {
  const { data: approachSettings } = useSiteSettings<ApproachContent>("approach");

  const label = approachSettings?.label || "Мой подход";
  const title = approachSettings?.title || "Не просто исполнитель, ";
  const titleAccent = approachSettings?.titleAccent || "а партнёр в росте";
  const subtitle = approachSettings?.subtitle || "Я не беру задачи «сделать пост» или «запустить рекламу». Работаю с теми, кому нужен маркетинг как система — с пониманием целей и ответственностью за результат.";
  const pillars = approachSettings?.pillars || defaultPillars;
